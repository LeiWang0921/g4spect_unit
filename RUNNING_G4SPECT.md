...
`g4spect_unit` 是当前 SPECT 探测器单元的最小 Geant4 原型。它模拟 Tc-99m gamma 光子从源出发，可选地先穿过 DICOM/CT voxel 人体 phantom，再经过可选 Pb 平行孔准直器，进入 LYSO 闪烁晶体后的输运和能量沉积。
...
# 加入 DICOM/CT voxel 人体 phantom，使用默认 1000 mm detector-front distance
G4VRMLFILE_MAX_FILE_NUM=1 ./G4SPECT --dicom-phantom ../mac/export_geometry_tracks_ShowAllBeam.mac

# 加入人体 phantom，并临时指定 detector 前表面距离源点 300 mm
...
```bash
G4SPECT_ENABLE_DICOM_PHANTOM=1 \
...
如果 Blender 导入时卡死，优先检查是否用 `G4SPECT_SHOW_DICOM_VOXELS=1` 导出了每个 voxel。默认不要打开这个环境变量；完整 voxel phantom 仍然参与 Geant4 tracking，只是不逐个 voxel 写入 VRML/OBJ 可视化文件。`DICOM_PhantomContainer` 是 Geant4 parameterised voxel phantom 的 mother volume，只代表包住所有 voxel 的 bounding box，不是人体轮廓；常规查看应看 `DICOM_PhantomPreview`。
...
cd /RHEL7/home/lwang2/g4spect_unit/build
source /opt/geant4_10.5/install/bin/geant4.sh
G4VRMLFILE_MAX_FILE_NUM=1 ./G4SPECT --dicom-phantom ../mac/export_geometry_tracks_ShowDetectedBeam.mac
...
- `include/SpectDetectorConstruction.hh`
  ...
  声明 `SpectDetectorConstruction`，负责构建 detector geometry、材料和敏感探测器。它保存 LYSO logical volume 指针、DICOM phantom enable flag 和当前材料指针，并声明 `BuildLYSO()`、`BuildOpticalGel()`、`DefineMaterials()`、`Construct()`、`ConstructSDandField()`。
...
- `G4SPECT.cc`
  ...
  程序入口。它解析 `--dicom-phantom` 和 `G4SPECT_ENABLE_DICOM_PHANTOM`，创建 `G4RunManager`，注册 detector construction、physics list 和 action initialization，初始化可视化管理器，并根据命令行参数决定进入 UI session 或执行指定 macro。
