...
`g4spect_unit` 是当前 SPECT 探测器单元的最小 Geant4 原型。它模拟 Tc-99m gamma 光子从源出发，经过 Pb 平行孔准直器，进入 LYSO 闪烁晶体后的输运和能量沉积。
...
- 准直器：`64 mm x 64 mm x 35 mm`，中心在 `z = 117.5 mm`
- 准直器前表面：`z = 100 mm`
- 准直器后表面：`z = 135 mm`
- 准直器孔阵列：`37 x 37` 个方孔
...
- 孔中心距：`1.7 mm`
- septum 厚度：约 `0.2 mm`
- LYSO：`64 mm x 64 mm x 10 mm`，中心在 `z = 140.0 mm`
- LYSO 前表面：`z = 135 mm`
- 光学胶：`64 mm x 64 mm x 0.5 mm`，中心在 `z = 145.25 mm`
- SiPM：`64 mm x 64 mm x 1 mm`，中心在 `z = 146.0 mm`

准直器不是用 `G4SubtractionSolid` 从一个大 Pb 块里逐孔 subtract 出来的。当前实现是一个
...
- `include/SpectDetectorConstruction.hh`
  ...
  声明 `SpectDetectorConstruction`，负责构建 detector geometry、材料和敏感探测器。它保存 LYSO logical volume 指针和当前材料指针，并声明 `BuildLYSO()`、`BuildOpticalGel()`、`DefineMaterials()`、`Construct()`、`ConstructSDandField()`。
...
- `src/SpectDetectorConstruction.cc`
  ...
  实现当前所有几何和材料。它创建 world、LYSO、optical gel、SiPM 和 Pb parallel-hole collimator；准直器用 air envelope 加 placed Pb border/septa 条来形成 air holes，避免大规模 nested `G4SubtractionSolid`。它还把 LYSO 设置为敏感体，并设置各部件可视化属性。
...
- `xpos/ypos/zpos` 是 Geant4 world coordinate 中的 hit 位置，不是晶体像素编号。当前几何中 LYSO 的中心在 `(0, 0, 140 mm)`，横向尺寸是 `64 mm x 64 mm`，所以 LYSO 内大多数记录的 `xpos` 和 `ypos` 会落在大约 `-32 mm` 到 `+32 mm` 之间，`zpos` 会落在大约 `135 mm` 到 `145 mm` 之间。
...
- 例如 `tc99m_point_iso.mac` 中 `/run/beamOn 10000` 表示 Geant4 生成了 `10000` 个 primary gamma event。由于各向同性源被准直器大量拒绝，绝大多数 event 没有任何 LYSO hit，因此不会在 `tree_chroma` 中出现。
