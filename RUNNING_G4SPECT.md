...
`g4spect_unit` 是当前 SPECT 探测器单元的最小 Geant4 原型。它模拟 Tc-99m gamma 光子从源出发，可选地先穿过简化 MIRD-style 人形 soft-tissue phantom 或 DICOM/CT voxel 人体 phantom，再经过可选 Pb 平行孔准直器，进入 LYSO 闪烁晶体后的输运和能量沉积。

当前 Geant4 阶段不产生光学光子。它只记录 LYSO 中的能量沉积和粒子 step 信息。后续计划是把这些 LYSO 能量沉积结果交给 Chroma，由 Chroma 负责闪烁光子的产生、传播、界面光学和 SiPM 探测。

当前几何顺序沿 `+z` 方向为：

```text
Tc-99m gamma 源
  -> 可选简化 MIRD-style 人形 phantom 或 DICOM/CT voxel 人体 phantom
  -> 可选 Pb 平行孔准直器
  -> LYSO 晶体
  -> optical gel
  -> SiPM slab
...
- World：`G4_AIR`
- 准直器：`G4_Pb`
- LYSO：自定义 `LYSO`，密度 `7.10 g/cm3`
- 光学胶：自定义 `OpticalGel`，密度 `1.03 g/cm3`
- SiPM：`G4_Si`
- 简化 MIRD-style 人形 phantom：优先使用 `G4_TISSUE_SOFT_ICRP`，如果该材料不可用则退回 `G4_WATER`

当前尺寸是一个局部临床化 detector patch，不是完整临床相机的几十厘米级 FOV：

- 源位置：`(0, 0, 0 mm)`
- 探测器横向尺寸：`64 mm x 64 mm`
- 探测器前表面距离源点：由 `G4SPECT_DETECTOR_DISTANCE_MM` 控制，默认 `1000 mm`
- 准直器：`64 mm x 64 mm x 35 mm`，中心在 `z = detectorFrontZ + 17.5 mm`
- 准直器前表面：`z = detectorFrontZ`
- 准直器后表面：`z = detectorFrontZ + 35 mm`
- 准直器孔阵列：`37 x 37` 个方孔
- 单孔孔径：`1.5 mm x 1.5 mm`
- 孔中心距：`1.7 mm`
- septum 厚度：约 `0.2 mm`
- LYSO：`64 mm x 64 mm x 10 mm`，中心在 `z = detectorFrontZ + 40 mm`
- LYSO 前表面：`z = detectorFrontZ + 35 mm`
- 光学胶：`64 mm x 64 mm x 0.5 mm`，中心在 `z = detectorFrontZ + 45.25 mm`
- SiPM：`64 mm x 64 mm x 1 mm`，中心在 `z = detectorFrontZ + 46.0 mm`

如果临时设置 `G4SPECT_DETECTOR_DISTANCE_MM=100`，这些位置会回到旧版几何的近场检查距离；默认 `1000 mm` 用于更接近远距离响应检查。注意：临床 SPECT 中 detector head 通常会尽量靠近人体表面，真实源到准直器距离应由具体设备和人体位置决定，`1000 mm` 只是当前默认基线，不是固定物理常数。

当前默认不构建准直器，因为 placed Pb septa 的 volume 数量较多，VRML/OBJ 导出和 Blender 导入会占用大量内存。默认几何顺序实际是：
...
```bash
G4SPECT_ENABLE_COLLIMATOR=1 ./G4SPECT ../mac/tc99m_beam_smoke.mac
```

打开准直器后，准直器占据 `detectorFrontZ` 到 `detectorFrontZ + 35 mm`，LYSO 前表面移动到 `detectorFrontZ + 35 mm`。默认不打开准直器时，LYSO 前表面就是 `detectorFrontZ`。

可选 DICOM/CT voxel 人体 phantom：

- 启用方式：命令行加 `--dicom-phantom`，或设置 `G4SPECT_ENABLE_DICOM_PHANTOM=1`
- 默认数据：`/opt/geant4_10.5/examples/extended/medical/DICOM/1.g4dcm,2.g4dcm,3.g4dcm`
...
- 默认可视化：默认不显示每个 DICOM voxel，也不把 `DICOM_PhantomContainer` 那块 mother-volume 薄板作为人体轮廓使用；`vrml_to_obj.py` 会根据非空气 voxel 生成一个合并后的低分辨率 `DICOM_PhantomPreview` 轮廓对象
...
注意：默认 example 只有 `1.g4dcm,2.g4dcm,3.g4dcm` 三个 slice，所以即使用 `DICOM_PhantomPreview`，看到的也只是一个薄 CT 切片堆的轮廓，不会像完整人体模型那样有三维人体外形。

可选简化 MIRD-style 人形 phantom：

- 启用方式：命令行加 `--human-phantom`，或设置 `G4SPECT_ENABLE_HUMAN_PHANTOM=1`
- 几何用途：作为人体 attenuation/scatter geometry 参与 gamma tracking，不作为 sensitive detector
- 输出：ROOT 输出仍然只记录 LYSO 中的 `tree_chroma` hit/step 信息
- 外形：半透明、可识别的人体外轮廓，只包含躯干、头部、左右手臂和左右腿；不包含内部 organ 或 CT voxel
- 材料：均匀 soft tissue，优先 `G4_TISSUE_SOFT_ICRP`，找不到时使用 `G4_WATER`
- 坐标：人体沿 `y` 方向站立，`z` 仍然是源点到 detector 的方向；源点 `(0, 0, 0 mm)` 位于躯干中心附近
- 典型尺寸：躯干约 `40 cm x 20 cm x 70 cm`，整体高度约 `1.7 m`

这个模型可以理解为“人形态的组织/水等效体”。它比 Geant4 example 的三切片 DICOM phantom 更适合当前 Blender 可视化和快速 attenuation/scatter 测试，但它不是病人 CT，也不包含器官材料分布。`--human-phantom` 和 `--dicom-phantom` 不能在同一次运行中同时启用。

可选准直器不是用 `G4SubtractionSolid` 从一个大 Pb 块里逐孔 subtract 出来的。当前实现是一个
air envelope，里面放置 Pb border 和 septa 条；孔洞就是没有 Pb 的 air gap。这种写法比
大规模 nested boolean subtraction 更适合后续 VRML 可视化和 Geant4 导航。
...
是否加入人体 phantom 与 macro 文件本身无关，由 `G4SPECT` 启动参数决定：

```bash
# 不加入人体 phantom
G4VRMLFILE_MAX_FILE_NUM=1 ./G4SPECT ../mac/export_geometry_tracks_ShowAllBeam.mac

# 加入简化 MIRD-style 人形 phantom，使用默认 1000 mm detector-front distance
G4VRMLFILE_MAX_FILE_NUM=1 ./G4SPECT --human-phantom ../mac/export_geometry_tracks_ShowAllBeam.mac

# 加入 DICOM/CT voxel 人体 phantom，主要用于 voxel/CT 链路检查
G4VRMLFILE_MAX_FILE_NUM=1 ./G4SPECT --dicom-phantom ../mac/export_geometry_tracks_ShowAllBeam.mac

# 加入简化 MIRD-style 人形 phantom，并临时指定 detector 前表面距离源点 300 mm
G4VRMLFILE_MAX_FILE_NUM=1 G4SPECT_DETECTOR_DISTANCE_MM=300 \
  ./G4SPECT --human-phantom ../mac/export_geometry_tracks_ShowAllBeam.mac

# 加入简化 MIRD-style 人形 phantom，同时恢复准直器
...
```bash
G4SPECT_ENABLE_HUMAN_PHANTOM=1 \
G4VRMLFILE_MAX_FILE_NUM=1 \
./G4SPECT ../mac/export_geometry_tracks_ShowAllBeam.mac
```

DICOM/CT voxel phantom 的等价环境变量是 `G4SPECT_ENABLE_DICOM_PHANTOM=1`。不要同时设置 `G4SPECT_ENABLE_HUMAN_PHANTOM=1` 和 `G4SPECT_ENABLE_DICOM_PHANTOM=1`。

### 1. 运行 ShowAllBeam

`export_geometry_tracks_ShowAllBeam.mac` 使用位于 `(0, 0, 0 mm)` 的 isotropic Tc-99m
gamma source，运行 `/run/beamOn 100`，并把所有保留下来的轨迹累积到 VRML 中。
它会导出探测器几何、源位置坐标轴和这些 generated events 的轨迹。
用于人体 phantom 3D 预览时，这个 macro 应保持 `/gps/ang/type iso`，这样能检查源点位于人体内时向全空间发射的几何关系。
...
如果要检查 DICOM/CT voxel phantom 链路，把 `--human-phantom` 换成 `--dicom-phantom`。

默认 `G4SPECT_DETECTOR_DISTANCE_MM=1000` 时，`ShowAllBeam` 的 100 个 isotropic event 多数不会打到 64 mm detector patch，这不是错误。这个视图主要用于检查人体 phantom、源点、探测器和发散轨迹的相对位置。
...
如果 Blender 导入时卡死，优先检查是否用 `G4SPECT_SHOW_DICOM_VOXELS=1` 导出了每个 voxel。默认不要打开这个环境变量；完整 voxel phantom 仍然参与 Geant4 tracking，只是不逐个 voxel 写入 VRML/OBJ 可视化文件。`DICOM_PhantomContainer` 是 Geant4 parameterised voxel phantom 的 mother volume，只代表包住所有 voxel 的 bounding box，不是人体轮廓；常规查看应看 `DICOM_PhantomPreview`。如果目标只是看可识别的人体外形，优先使用 `--human-phantom`，它只导出少量外轮廓对象，Blender 负担小很多。
...
`export_geometry_tracks_ShowDetectedBeam.mac` 使用位于 `(0, 0, 0 mm)` 的 source，
并用 directed beam 沿 `(0, 0, +1)` 穿过中心孔。随后它用 Geant4 的
`encounteredVolumeFilter` 按 `LYSO_phys` 过滤轨迹。它运行 `/run/beamOn 100`，
突出显示遇到 LYSO 的轨迹。

这个 macro 是 detected-track 可视化检查用，不用于估计真实各向同性点源的探测效率。真实各向同性运行仍然使用
`tc99m_point_iso.mac`。如果你想看全空间随机方向中有哪些轨迹被准直器接受，需要另建或临时修改
macro；直接用 10000 个全空间 isotropic event 经常不会产生足够的 detected tracks。
...
带人体 phantom 的 detected-track 可视化命令是：

```bash
cd /RHEL7/home/lwang2/g4spect_unit/build
source /opt/geant4_10.5/install/bin/geant4.sh
G4VRMLFILE_MAX_FILE_NUM=1 ./G4SPECT --human-phantom ../mac/export_geometry_tracks_ShowDetectedBeam.mac

python3 ../tools/vrml_to_obj.py
```

如果默认 1 m 距离下想更容易看清轨迹末端结构，可以先用 `ShowDetectedBeam` 的 directed beam 做几何检查；如果想快速放大近场结构，可以临时加 `G4SPECT_DETECTOR_DISTANCE_MM=100` 运行，但这种结果只作为调试视图。
...
Blender 材质透明度。当前 `LYSO_phys` 闪烁体和 `OpticalGel_phys` 光学胶都应该显示为
...
`0.005 mm`；如果还想调整，可以重新转换时指定不同半径，例如：
...
- `event_001`、`event_002`、…… 分别对应被 VRML 导出的 generated events。
- `G4AxesModel...` 是 `/vis/scene/add/axes 0 0 0 10 mm` 生成的源位置坐标轴，不是 gamma 轨迹。
- `gamma_primary_*` 是 primary gamma 轨迹。
...
放射源位置用一组小坐标轴标在 `(0, 0, 0 mm)`。普通 Geant4 几何导出不会显示
...
各向同性点源运行的有效命中率会明显低于 directed beam smoke test。默认不构建准直器时，主要原因是 `64 mm x 64 mm` LYSO patch 在 `1000 mm` 距离处张角很小；如果设置 `G4SPECT_ENABLE_COLLIMATOR=1`，准直器还会进一步拒绝绝大多数非准直方向的 gamma。这种低命中率可能是物理上合理的，不能直接当成程序错误。
...
- `include/SpectDetectorConstruction.hh`
  ...
  声明 `SpectDetectorConstruction`，负责构建 detector geometry、材料和敏感探测器。它保存 LYSO logical volume 指针、DICOM phantom enable flag、MIRD-style human phantom enable flag 和当前材料指针，并声明 `BuildLYSO()`、`BuildOpticalGel()`、`DefineMaterials()`、`Construct()`、`ConstructSDandField()`。

- `include/SpectDicomPhantom.hh`

  声明 `SpectDicomPhantom`，负责把 Geant4 example 的 `.g4dcm` voxel phantom 数据作为可选人体 attenuation/scatter geometry 加入 world。它不声明 sensitive detector，也不改变 ROOT 输出字段。

- `include/SpectMirdHumanPhantom.hh`

  声明 `SpectMirdHumanPhantom`，负责把简化 MIRD-style 人形 soft-tissue outer geometry 加入 world。它只声明外形 phantom 构建接口，不声明 sensitive detector，也不改变 ROOT 输出字段。

- `include/SpectEventAction.hh`

  声明 `SpectEventAction`，负责在每个 event 结束时读取 LYSO hits collection，并把 hit 信息写入 Geant4 analysis ntuple。它缓存 `lysoHitsCollection` 的 collection id。

- `include/SpectHit.hh`

  声明单个 LYSO hit 的数据结构 `SpectHit`，包括能量沉积、位置、时间、粒子名、PDG 编码、粒子动能、动量和 track id。文件末尾定义 `SpectHitsCollection`，即 `G4THitsCollection<SpectHit>`。
...
- `include/SpectSensitiveDetector.hh`
  ...
  声明 `SpectSensitiveDetector`，即挂在 LYSO logical volume 上的敏感探测器。它负责初始化 hits collection、处理每个 step，并在 event 结束时把 collection 加入 `G4HCofThisEvent`。
...
- `src/SpectDetectorConstruction.cc`
  ...
  实现当前 detector 几何和材料。它创建 world、LYSO、optical gel 和 SiPM；当 `--human-phantom` 或 `G4SPECT_ENABLE_HUMAN_PHANTOM=1` 启用时，它调用 `SpectMirdHumanPhantom` 加入简化 MIRD-style 人形 phantom；当 `--dicom-phantom` 或 `G4SPECT_ENABLE_DICOM_PHANTOM=1` 启用时，它调用 `SpectDicomPhantom` 加入 DICOM/CT voxel 人体 phantom；当 `G4SPECT_ENABLE_COLLIMATOR=1` 启用时，它再创建 Pb parallel-hole collimator。准直器用 air envelope 加 placed Pb border/septa 条来形成 air holes，避免大规模 nested `G4SubtractionSolid`。它还把 LYSO 设置为敏感体，并设置各部件可视化属性。探测器前表面距离由 `G4SPECT_DETECTOR_DISTANCE_MM` 控制，默认 `1000 mm`。
...
- `src/SpectMirdHumanPhantom.cc`

  实现简化 MIRD-style 人形 phantom。当前使用半透明 soft-tissue/water-equivalent 外形体积：躯干、头部、左右手臂和左右腿；人体沿 `y` 方向站立，源点位于躯干中心附近，`z` 仍是源点到 detector 的方向。该 phantom 只参与 gamma attenuation/scatter，不作为 sensitive detector。

- `src/SpectEventAction.cc`

  在 event 结束时遍历 `lysoHitsCollection`，把每个 `SpectHit` 转换成 `tree_chroma` 的一行。它负责单位转换：位置写成 `mm`，时间写成 `ns`，能量和动量相关量写成 `keV`。它还把粒子名映射成当前的数值索引。
...
- `src/SpectSensitiveDetector.cc`
  ...
  实现 LYSO sensitive detector。每个 event 初始化 `lysoHitsCollection`；对非 optical photon 的 track 记录 first-step hit 和后续 step hit；每个 hit 保存 energy deposition、post-step 位置、local time、粒子信息、动量和 track id。当前 optical photon 会被忽略。

### 入口和构建文件

- `G4SPECT.cc`

  程序入口。它解析 `--human-phantom`、`G4SPECT_ENABLE_HUMAN_PHANTOM`、`--dicom-phantom` 和 `G4SPECT_ENABLE_DICOM_PHANTOM`，并禁止同时启用两个人体 phantom。随后它创建 `G4RunManager`，注册 detector construction、physics list 和 action initialization，初始化可视化管理器，并根据命令行参数决定进入 UI session 或执行指定 macro。
...
`tree_chroma` 记录 LYSO 中的 hit/step 信息，当前列包括：
...
每一行代表 LYSO sensitive detector 记录到的一条 hit/step 记录。当前实现只记录非 optical photon 的 track；每个 track 在第一步会额外写入一条 `edep = 0` 的 first-hit 记录，用来保存该 track 进入记录流程时的 pre-step 状态，随后每个 Geant4 step 再写入一条常规记录。
...
| `hits` | 无 | 当前 event 内 `lysoHitsCollection` 的条目序号。它是 hit 行索引，不是该 event 的总 hit 数。 |
...
- `hits = 30` 表示这行是该 event 的 `lysoHitsCollection` 中第 `30` 个条目。这个索引同样从 `0` 开始，所以它是该 event 中第 `31` 条被写出的 LYSO hit/step 记录。它不是这个 event 的总 hit 数，也不是第 30 个 SiPM 像素或第 30 个准直孔。
- 同一个 `event` 可以对应多行 ROOT 记录，因为一个 primary gamma 及其产生的次级粒子可能在 LYSO 中产生多个 step。分析时通常要按 `event` 分组，再在组内查看 `hits`、`trackID`、`edep` 和位置。
- `xpos/ypos/zpos` 是 Geant4 world coordinate 中的 hit 位置，不是晶体像素编号。当前几何中 LYSO 横向尺寸是 `64 mm x 64 mm`，所以 LYSO 内大多数记录的 `xpos` 和 `ypos` 会落在大约 `-32 mm` 到 `+32 mm` 之间。默认不构建准直器时，LYSO 的 z 范围是 `G4SPECT_DETECTOR_DISTANCE_MM` 到 `G4SPECT_DETECTOR_DISTANCE_MM + 10 mm`；默认距离 `1000 mm` 时约为 `1000 mm` 到 `1010 mm`。如果启用准直器，LYSO 会再向后移动 `35 mm`。
- `xpos` 的物理意义是 hit 在 LYSO 横向平面中的 x 位置。`xpos = 0 mm` 表示靠近探测器横向中心；`xpos > 0` 和 `xpos < 0` 分别表示中心两侧的两个 x 方向。当前代码没有把 x/y 离散化成 SiPM channel 或 collimator hole index。
...
- ROOT histogram 右上角的 `Entries` 是这张图被填入的条目数。对 `tree_chroma->Draw("event")` 来说，它等于 `tree_chroma` 中被画出的行数，也就是 LYSO hit/step 记录数，不是 Geant4 生成的 primary gamma 数。
- 例如 `tc99m_point_iso.mac` 中 `/run/beamOn 10000` 表示 Geant4 生成了 `10000` 个 primary gamma event。默认远距离小 detector patch 的几何张角很小；如果启用准直器，还会有额外准直器接受角限制。因此绝大多数 event 可能没有任何 LYSO hit，不会在 `tree_chroma` 中出现。
- 如果 `event` 图显示 `Entries = 283`，含义是 ROOT 文件里有 `283` 行 LYSO hit/step 记录；它不表示只生成了 `283` 个 gamma，也不表示有 `283` 个不同 event。
- `event` 图上只有少数几个 bin 有计数，是因为只有少数 event ID 产生了 LYSO 记录。某个 event 如果在 LYSO 中产生很多 step，它会贡献很多行，因此同一个 event ID 会在 histogram 中被重复计数，形成较高的柱。
...
- 各向同性点源命中少时，先检查 `G4SPECT_DETECTOR_DISTANCE_MM`、detector patch 张角，以及是否启用了 `G4SPECT_ENABLE_COLLIMATOR`，不要马上判断为 bug。
